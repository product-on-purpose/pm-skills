...
def generate_thread_page(thread_key: str) -> None:
    ...
    lines.append("source: scripts/generate-showcase.py")
    lines.append("tags:")
    ...
    lines.append('This page is regenerated by `scripts/generate-showcase.py` from `library/skill-output-samples/`. Re-run the generator after updating samples or thread definitions.')
    lines.append(':::')
    lines.append("")
    ...
    for phase_key, phase_display in PHASES:
        ...
        for skill in skills:
            ...
            for sample_file in sample_files:
                ...
                if parsed["prompt"]:
                    phase_lines.append(':::note[Prompt]')
                    phase_lines.append(parsed["prompt"])
                    phase_lines.append(':::')
                    phase_lines.append("")

                # Output in MDX <details> collapsible
                phase_lines.append('<details>')
                phase_lines.append(f'<summary>Full output: {skill_display}{variant_info}</summary>')
                phase_lines.append("")
                phase_lines.append(parsed["output"])
                phase_lines.append("")
                phase_lines.append('</details>')
                phase_lines.append("")
                phase_lines.append("---")
...
def generate_index_page() -> None:
    ...
    lines.append("")
    lines.append(':::caution[Generated file]')
    lines.append('This page is regenerated by `scripts/generate-showcase.py`. Re-run the generator after updating thread definitions or sample library entries.')
    lines.append(':::')
    lines.append("")
