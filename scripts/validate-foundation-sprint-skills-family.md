# validate-foundation-sprint-skills-family.sh / validate-foundation-sprint-skills-family.ps1

## Purpose

Validate conformance of the 7 `tool-foundation-sprint-*` skills to the Foundation Sprint Skills Family Contract at `docs/reference/skill-families/foundation-sprint-skills-contract.md`. Complements (does not duplicate) `lint-skills-frontmatter`, which handles universal per-skill frontmatter checks. This script validates the **family-specific** structural and reference requirements shared across the Foundation Sprint family (introduced v2.15.0 under the `tool` classification).

## Usage

```bash
./scripts/validate-foundation-sprint-skills-family.sh
./scripts/validate-foundation-sprint-skills-family.sh --strict
```

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\validate-foundation-sprint-skills-family.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\validate-foundation-sprint-skills-family.ps1 -Strict
```
...
- Canonical contract exists at `docs/reference/skill-families/foundation-sprint-skills-contract.md`
...
| `metadata.move` | Matches the skill's expected move slug (e.g., `tool-foundation-sprint-basics` -> `basics`, `tool-foundation-sprint-founding-hypothesis` -> `founding-hypothesis`) |
| SKILL.md references contract | `SKILL.md` contains the family-contract path `docs/reference/skill-families/foundation-sprint-skills-contract.md` |
...
- Canonical contract: `docs/reference/skill-families/foundation-sprint-skills-contract.md`
- Family registry: `docs/reference/skill-families/_registry.yaml`
- Sibling validator: `validate-design-sprint-skills-family` (parallel structure for the Design Sprint family)
