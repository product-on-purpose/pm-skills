...
test('the version marker constants are distinct from the PR1-PR4 markers and self-describe their edit target', () => {
  assert.equal(VERSION_BADGE_START, '<!-- pmskills:version-badge:start (generated by scripts/gen-derived-surfaces.mjs; edit .claude-plugin/plugin.json, not this block) -->');
  assert.equal(VERSION_BADGE_END, '<!-- pmskills:version-badge:end -->');
  assert.equal(VERSION_ROW_START, '<!-- pmskills:version-row:start (generated by scripts/gen-derived-surfaces.mjs; edit .claude-plugin/plugin.json, not this block) -->');
  assert.equal(VERSION_ROW_END, '<!-- pmskills:version-row:end -->');
});

test('renderVersionBadge round-trips the version into the badge URL, alt text stays fixed, and the old annotation comment is gone', () => {
  const line = renderVersionBadge('9.9.9');
  assert.ok(line.includes('badge/version-9.9.9-blue.svg?style=flat-square'));
  assert.ok(line.includes('alt="Version"'));
  assert.ok(!line.includes('x-release-please'), 'no trace of the retired annotation comment');
  assert.ok(line.endsWith('\n'));
});

const VERSION_ROW_FIX = [
  '|                     |                                                                                           |',
  '| ------------------- | ----------------------------------------------------------------------------------------- |',
  '| **Current version** | [v9.0.0](https://github.com/product-on-purpose/pm-skills/releases/tag/v9.0.0)           |',
  '| **Skill count**     | 68 skills (30 phase + 11 foundation + 12 utility + 15 tool)                               |',
  '| **Sub-agents**      | 6 (pm-critic, pm-skill-auditor, pm-changelog-curator, pm-release-conductor)                |',
].join('\n');

test('renderVersionRowTable rewrites only the Current-version row (both its version tokens), preserving every other row verbatim', () => {
  const out = renderVersionRowTable(VERSION_ROW_FIX, '9.9.9');
  const lines = out.split('\n');
  const fixLines = VERSION_ROW_FIX.split('\n');
  assert.ok(lines[2].includes('[v9.9.9](https://github.com/product-on-purpose/pm-skills/releases/tag/v9.9.9)'), 'both version tokens on the row are rewritten');
  assert.ok(!lines[2].includes('9.0.0'));
  assert.equal(lines[0], fixLines[0], 'header row untouched');
  assert.equal(lines[1], fixLines[1], 'separator row untouched');
  assert.equal(lines[3], fixLines[3], 'Skill count row untouched');
  assert.equal(lines[4], fixLines[4], 'Sub-agents row untouched');
});

test('renderVersionRowTable is idempotent when the version already matches (a re-render is a no-op)', () => {
  assert.equal(renderVersionRowTable(VERSION_ROW_FIX, '9.0.0'), VERSION_ROW_FIX);
});

test('renderVersionRowTable fails loudly when the Current-version row is missing', () => {
  const noRow = '| **Skill count** | 68 skills |';
  assert.throws(() => renderVersionRowTable(noRow, '9.9.9'), /"Current version" row not found/);
});

test('--check passes on a matching version-badge region and trips on a hand edit', () => {
  const block = renderVersionBadge('9.9.9');
  const doc = `intro\n\n${VERSION_BADGE_START}\n${block}${VERSION_BADGE_END}\n\noutro\n`;
  assert.equal(normalizeEol(doc), normalizeEol(splice(doc, VERSION_BADGE_START, VERSION_BADGE_END, block)));
  const tampered = doc.replace('9.9.9', '1.2.3');
  assert.notEqual(
    normalizeEol(tampered),
    normalizeEol(splice(tampered, VERSION_BADGE_START, VERSION_BADGE_END, block)),
  );
});

test('--check does not read false-stale for a CRLF version-row table (Windows checkout)', () => {
  const block = renderVersionRowTable(VERSION_ROW_FIX, '9.0.0'); // already-current, so this is a no-op render
  const lf = `intro\n\n${VERSION_ROW_START}\n${block}${VERSION_ROW_END}\n\noutro\n`;
  const crlf = lf.replace(/\n/g, '\r\n');
  // Mirror main()'s read path: slice the current block out of the CRLF doc, re-render, re-splice.
  const s = crlf.indexOf(VERSION_ROW_START);
  const e = crlf.indexOf(VERSION_ROW_END);
  const existingBlock = crlf.slice(s + VERSION_ROW_START.length, e);
  const rendered = renderVersionRowTable(existingBlock, '9.0.0');
