# Validate design-sprint-skills family contract conformance across the
...
# family-specific structural and reference requirements for the Design
# Sprint family.
#
# See: docs/reference/skill-families/design-sprint-skills-contract.md
...
#
# Partial-state behavior: between 1 and 6 of 7 skills authored, the validator
# emits WARN by default; -Strict elevates to FAIL (release-time CI invocation).
...
$ExpectedMove = @{
    "tool-design-sprint-readiness"             = "readiness"
    "tool-design-sprint-brief"                 = "brief"
    "tool-design-sprint-map-and-target"        = "map-and-target"
    "tool-design-sprint-sketch"                = "sketch"
    "tool-design-sprint-decide-and-storyboard" = "decide-and-storyboard"
    "tool-design-sprint-prototype-plan"        = "prototype-plan"
    "tool-design-sprint-test-and-score"        = "test-and-score"
}
...
if ($AuthoredCount -eq 0) {
    Write-Host "[NOTICE] design-sprint-skills-family : 0 of $($FamilySkills.Count) family skills authored yet (scaffolding phase)."
    Write-Host "[NOTICE] design-sprint-skills-family : contract presence + per-skill checks deferred until skills land."
    Write-Host "[NOTICE] design-sprint-skills-family : validator exits 0 in scaffolding state."
    exit 0
}
...
if ($AuthoredCount -lt $TotalCount) {
    if ($Strict) {
        Write-Host "[FAIL] design-sprint-skills-family : $AuthoredCount of $TotalCount family skills authored. Strict mode requires all $TotalCount to ship."
        $Fail = 1
    } else {
        Write-Host "[WARN] design-sprint-skills-family : $AuthoredCount of $TotalCount family skills authored. Family is incomplete. Pass -Strict to fail on partial state (recommended for release-time validation)."
    }
}

Write-Host "[INFO] design-sprint-skills-family : $AuthoredCount of $TotalCount family skills present; enforcing checks."
...
foreach ($skill in $FamilySkills) {
    ...
    if ($metaTool -ne 'design-sprint') {
        FailSkill $skill "metadata.tool='$metaTool' (expected 'design-sprint')"
    } else {
        PassSkill $skill "metadata.tool: design-sprint"
    }
...
Write-Host ""
if ($Fail -eq 0) {
    Write-Host "All design-sprint-skills-family contract checks passed."
    exit 0
} else {
    Write-Host "Design-sprint-skills-family contract checks FAILED. See '[FAIL]' lines above."
    exit 1
}
