...
if [[ $authored_count -eq 0 ]]; then
  echo "[NOTICE] design-sprint-skills-family : 0 of ${#FAMILY_SKILLS[@]} family skills authored yet (scaffolding phase)."
  echo "[NOTICE] design-sprint-skills-family : contract presence + per-skill checks deferred until skills land."
  echo "[NOTICE] design-sprint-skills-family : validator exits 0 in scaffolding state."
  exit 0
fi
...
if [[ $authored_count -lt $total_count ]]; then
  if [[ $STRICT -eq 1 ]]; then
    echo "[FAIL] design-sprint-skills-family : $authored_count of $total_count family skills authored. Strict mode requires all $total_count to ship."
    FAIL=1
  else
    echo "[WARN] design-sprint-skills-family : $authored_count of $total_count family skills authored. Family is incomplete. Pass --strict to fail on partial state (recommended for release-time validation)."
  fi
fi

echo "[INFO] design-sprint-skills-family : $authored_count of $total_count family skills present; enforcing checks."
...
for skill in "${FAMILY_SKILLS[@]}"; do
  ...
  ')
  if [[ "$meta_tool" != "design-sprint" ]]; then
    fail_skill "$skill" "metadata.tool='$meta_tool' (expected 'design-sprint')"
  else
    pass_skill "$skill" "metadata.tool: design-sprint"
  fi
...
echo ""
if [[ $FAIL -eq 0 ]]; then
  echo "All design-sprint-skills-family contract checks passed."
  exit 0
else
  echo "Design-sprint-skills-family contract checks FAILED. See '[FAIL]' lines above."
  exit 1
fi
