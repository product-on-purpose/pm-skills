...
def generate_skill_page(skill_dir: Path) -> dict | None:
    ...
    lines.append(':::caution[Generated file]')
    lines.append(f'This page is regenerated by `scripts/generate-skill-pages.py`. Edit the source at `skills/{dirname}/SKILL.md` and re-run the generator.')
    lines.append(':::')
    lines.append("")

    # Quick facts aside
    lines.append(':::note[Quick facts]')
    ...
    if example_content:
        ...
        lines.append(f'<summary>{example_title}</summary>')
        lines.append("")
        lines.append(example_body.strip())
        lines.append("")
        lines.append('</details>')
        lines.append("")
    ...
    if samples:
        ...
        for sample in samples:
            ...
            lines.append('<details>')
            lines.append(f'<summary>{sample["thread_display"]}: {context_label}</summary>')
            lines.append("")
            if sample["prompt"]:
...
def generate_phase_index(group: str, skills: list) -> None:
    ...
    # Generated-content aside (Pattern 5C)
    lines.append(':::caution[Generated file]')
    lines.append(f'This page is regenerated by `scripts/generate-skill-pages.py` from the `skills/{group}/` directory. Re-run the generator after adding or changing skills.')
    lines.append(':::')
    lines.append("")
...
def generate_commands_reference(all_skills: list) -> None:
    ...
    lines.append("")
    lines.append(':::caution[Generated file]')
    lines.append('This page is regenerated by `scripts/generate-skill-pages.py` from the skill catalog and `commands/` directory. Re-run the generator after adding or changing skills or commands.')
    lines.append(':::')
    lines.append("")
