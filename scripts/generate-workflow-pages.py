...
def inject_generated_marker(content: str, source_file: str) -> str:
    """Inject Pattern 5C generated-content marker into frontmatter + admonition after H1.

    Adds 'generated: true' and 'source: scripts/generate-workflow-pages.py' to the
    frontmatter block, plus a !!! warning admonition pointing editors to the source
    workflow file. Idempotent: if the marker already exists, returns content unchanged.
    """
    ...
    # Inject admonition right after the first H1 heading
    h1_match = re.search(r"^(#\s+.+\n)", rest, re.MULTILINE)
    if h1_match:
        admonition = (
            '\n!!! warning "Generated file"\n'
            f'    This page is regenerated by `{generator}`. '
            f'Edit the source at `{source_file}` and re-run the generator.\n\n'
        )
        rest = rest[:h1_match.end()] + admonition + rest[h1_match.end():]
...
def generate_index(workflow_files: list[tuple[str, dict]]) -> str:
    ...
    lines = [
        ...
        "",
        '!!! warning "Generated file"',
        "    This page is regenerated by `scripts/generate-workflow-pages.py` from the `_workflows/` directory. Re-run the generator after adding or changing workflows.",
        "",
