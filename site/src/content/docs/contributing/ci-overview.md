...
- **Node on PATH.** In Git Bash on Windows, Node is not always on `PATH`. If `cd site && npm run build` or the node-based checks (`gen-site.mjs`, `verify-edit-links.mjs`, `check-rendered-links.mjs`) fail with `node: command not found`, prepend Node for the session: `export PATH="/c/nvm4w/nodejs:$PATH"` (Node 24). The bash validator scripts do not need Node; only the build + generator + link-check steps do.
