...
**Trust repair + hygiene.** A 2026-07-04 audit found this project's least-engineered surfaces carried its most damaging defects: the first sentence a new user reads contradicted its own sub-counts, the curated release zip omitted the `agents/` and `hooks/` directories the dispatch skills need at runtime, and `utility-pm-skill-builder` carried a hand-maintained inventory blind to 22% of the catalog. This release fixes every instance of that drift, closes the CI gate holes that let it pass (a new enforcing `check-count-phrases` gate plus an advisory `check-heading-canon` gate), and extends the same hygiene pass to headings, descriptions, dual-shell validators, the trigger-eval roster, and tool-family versioning (the 15 tool skills move 0.1.0 to 1.0.0). No new skills; catalog stays 68 skills / 6 sub-agents. Additive MINOR. Full details in root [`CHANGELOG.md`](https://github.com/product-on-purpose/pm-skills/blob/main/CHANGELOG.md#2300---2026-07-04) and [`docs/releases/Release_v2.30.0.md`](releases/Release_v2.30.0.md).
