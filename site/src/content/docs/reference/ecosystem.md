...
The PM-Skills ecosystem provides a file-based PM skill library plus an MCP server companion in maintenance mode. The current `pm-skills` repo ships 65 skill directories (30 phase skills, 9 foundation skills, 10 utility skills, 15 tool skills), while `pm-skills-mcp` (latest v2.9.3 in the v2.9.x maintenance line) embeds the full 40-skill catalog frozen at the v2.9.2 build.
...
|-----------|-------|-------------|
| Skills | 65 | Complete PM artifacts (30 phase skills + 9 foundation skills + 10 utility skills + 15 tool skills) |
...
All 65 skills are registered in `AGENTS.md` with corresponding slash commands in `commands/`. 12 workflows are in `_workflows/`.

#### Directory Structure

```
pm-skills/
├── skills/               # The 65 shipped skills (flat, including foundation, utility, tool)
...
├── commands/             # 10 command markdown files for Claude Code
...
| **Slash Commands** | ✅ 10 command docs | ❌ (use tools) |
