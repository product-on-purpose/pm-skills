...
**v2.26.0 smoke gate (release evidence gate, decision P-G):** the native-path smoke test (a `--dry-run` plus one live 2-step `/chain` run on the installed plugin) is a recorded evidence gate of the v2.26.0 release: it must RUN and its result must be recorded HERE before the v2.26.0 tag. It could not run in the build session itself (the plugin's sub-agents are not registered in a workspace-discovery session; installing the branch build requires the interactive `/plugin` flow), so the run is scheduled against main after the F-14/F-15 merge, before tagging. A recorded PASS removes the native-path EXPERIMENTAL caveat for Mode B chains; a recorded FAIL keeps EXPERIMENTAL and is disclosed in the release notes. Only an UNRUN test blocks the tag.
