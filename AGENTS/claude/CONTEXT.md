...
**Status:** v2.15.0 SHIPPED 2026-05-16 at HEAD `a108301` (annotated tag; GitHub Release published from `docs/releases/Release_v2.15.0.md`). Sprint Skills Launch: 15 new skills under the new `classification: tool` taxonomy (7 Foundation Sprint family + 7 Design Sprint family + 1 tool-note-and-vote standalone) implementing Knapp / Zeratsky / Kowitz canonical sprint methodologies. Skill catalog 40 to 55. Two family contracts (FS v0.3.0; DS v0.2.0; both with Naming Discipline section) with enforcing CI validators (each with --strict release mode). 3 new workflows including end-to-end FS+DS arc with narrative handoff conversation replacing the dropped bridge skill (12-row slot-mapping table + 3-question go/no-go checkpoint). 45 library samples (171 total) across 3 narrative threads (Brainshelf, Storevine, Workbench). Two operational user guides + two concept docs + 3 disambiguation/reference docs (workshop-sprints-vs-agile-sprints, sprint-methodology-glossary, workshop-method-comparison) + 8 sprint-family user guides (FAQ + cheat sheet + case studies + recovery playbook per family). Two adversarial-review cycles documented (FS-track 35 findings; DS-track 13 Codex + 20 self-audit; all P1 closed pre-tag).

**Post-tag closures shipped on main HEAD (not in v2.15.0 tag):** `f03d94d` regenerated 16 tool skill pages + 3 workflow pages + refreshed commands.md, patched check-count-consistency to exempt `tool` subset-descriptor. `c8ea6d9` post-tag artifact sweep + generate-workflow-pages fix (partial; the generator's hardcoded workflow_info dict was the v2.15.0 root cause for the workflow-index miss; closed in v2.15.1).

**v2.15.x audit (2026-05-16 evening; HEAD post-v2.15.1):** Self-review at `docs/internal/release-plans/v2.15.x/audit_v2.15.x_post-tag-self-review.md` surfaced 18 findings across docs drift / CI gaps / continuity / consistency / bugs. v2.15.1 patch shipped remediation for all P0+P1+P2+P3 findings plus 4 new CI validators (landing-page count assertion; workflow-generator coverage; AGENTS.md command-table sync; pre-tag validator bundle script).

**Skill count:** 55 (26 phase + 8 foundation + 6 utility + 15 tool). Foundation Sprint family is 7 of 7 LIVE; Design Sprint family is 7 of 7 LIVE; tool-note-and-vote standalone LIVE.
