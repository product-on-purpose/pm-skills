...
**Status:** v2.16.0 READY TO PUSH/TAG (content-complete on spike branch `feat/v2.16-astro-6-spike`; 14 commits ahead of `origin/feat/v2.16-astro-6-spike` as of 2026-05-17 late evening; all 27 enforcing validators PASS via `bash scripts/pre-tag-validate.sh`; 4 Codex adversarial review cycles closed; GATE B + GATE C cross-client tests PASSED on Codex CLI 0.128.0). Predecessor: v2.15.2 SHIPPED 2026-05-17 + v2.15.x CYCLE CLOSED (v2.15.0 `a108301` -> v2.15.1 `6f89439` -> v2.15.2 closeout).
...
**Skill count:** 59 in v2.16.0 (26 phase + 8 foundation + 10 utility + 15 tool). +4 from v2.15.2's 55: 4 new utility dispatch skills (utility-pm-critic, utility-pm-skill-auditor, utility-pm-changelog-curator, utility-pm-release-conductor). Foundation Sprint family is 7 of 7 LIVE; Design Sprint family is 7 of 7 LIVE; tool-note-and-vote standalone LIVE. Plus 4 sub-agents under new `subagents/` directory (Claude Code plugin runtime components; not counted in skills total).

**Validators in v2.16.0:** 27 truly enforcing (was 27 at v2.15.2; +1 net from extension of validate-agents-md to recognize `subagents/` directory per D19; -0 net since the extension was an extension not a new validator). Plus orchestration script `scripts/pre-tag-validate.{sh,ps1}` runs the full enforcing bundle. Full inventory + workflow mapping documented in new `docs/contributing/ci-overview.md` (added v2.16.0).
...
**Last Updated:** 2026-05-17 late evening (v2.16.0 content-complete on spike branch; CONTEXT.md refreshed for v2.16.0 ready-to-push state per Codex R13 sequencing rule "CONTEXT.md refresh runs LAST after all tracks land"). Previously updated 2026-05-10 (V12-V15 + A+B+C post-tag work; release docs hygiene sweep at end of v2.14.x cycle).
