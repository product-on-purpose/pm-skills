...
## [2.33.0](https://github.com/product-on-purpose/pm-skills/compare/v2.32.0...v2.33.0) (2026-09-05)
...
* **skills:** close all 3 round-11 findings; lowest round of the gate ([51d5bec](https://github.com/product-on-purpose/pm-skills/commit/51d5bec64e52febbad605275ccbbce997c7f01d7))
* **skills:** close all 4 round-7 findings; lowest round of the gate ([7996d00](https://github.com/product-on-purpose/pm-skills/commit/7996d00ed92386df94aa2b2e1d8723c5181d9222))
* **skills:** close all 4 round-8 findings; rewrite the trace block symmetrically (D20, D21) ([a32a7ab](https://github.com/product-on-purpose/pm-skills/commit/a32a7abdc902b4fe616f5fd499eb3e03a782b596))
* **skills:** close all 5 round-10 findings; the scope boundaries held ([e6e53ba](https://github.com/product-on-purpose/pm-skills/commit/e6e53ba72a010cbaa04c181c58c8cc81efbafc69))
* **skills:** close round 12; the fix that enforced one-claim-per-row broke it ([6973984](https://github.com/product-on-purpose/pm-skills/commit/697398434f11feea97120e9facc7d4ebabaa0151))
* **skills:** close round 6's fix-pass damage; defer 3 pre-existing (D18, D19) ([43769da](https://github.com/product-on-purpose/pm-skills/commit/43769da43c97eb56a1cd1f4efab9cdcdd9d0efa5))
* **skills:** D22 deletes the trace choreography; close all 5 round-9 findings ([a164878](https://github.com/product-on-purpose/pm-skills/commit/a1648780474e609e7550c88aeae0c5268115722d))
* **skills:** Kano fallback contradicted the repo's own sample-size contract ([4f5d79e](https://github.com/product-on-purpose/pm-skills/commit/4f5d79edc275d7f63bcdbb5485ae8263d36584ec))
* **skills:** resolve all 6 round-4 findings; the trend turns ([e27476a](https://github.com/product-on-purpose/pm-skills/commit/e27476a75847bc323848f4e3e5f143892fb65a25))
* **skills:** resolve all 6 round-5 findings; three were pre-existing ([7605947](https://github.com/product-on-purpose/pm-skills/commit/76059476510aadeeca6cc27684b0ef1ab9ede2e1))
* **skills:** resolve all 8 round-3 findings; D15=A, D16=B ([2b13dc2](https://github.com/product-on-purpose/pm-skills/commit/2b13dc29f608c175f22e75ae47a306aeb6482c04))
* **skills:** resolve G1 round 2; rule D12, D13 and D14 ([61e0d9e](https://github.com/product-on-purpose/pm-skills/commit/61e0d9e0d19eff093b84db24549f52f7d181190d))
...
**The release users notice.** v2.32.0 was an infrastructure cycle that shipped one opt-in feature the front door never mentioned, while three user-filed defects sat untracked for two weeks. This one corrects that balance. All three field-reported defects are fixed at the shape their reporters suggested: `foundation-persona` 2.6.1 regenerates its example as a filled-in instance of its own template, verified structurally (13 of 13 sections, none added, no leaked authoring blockquotes, no unfilled placeholders) rather than by eye, and the existing persona is re-rendered rather than replaced; `define-prioritization-framework` 1.3.0 scales its top-and-bottom rule at ten or fewer items, redefines RICE Effort as capacity-weeks against real team capacity with the conversion stated, and gives Kano surveyed and inferred evidence tiers set by collection method, with sample adequacy as a separate gate, so the skill refuses only when there is no research at all; and the cross-skill handoff pointers are corrected against a precise signal (a sibling named inside an output, routing, or refusal section) after measurement narrowed the scope from fourteen skills to four, the other ten being regex noise exactly as the recorded residual risk predicted. Project memory finally gets a front door: four surfaces now point at it (README, site index, quickstart, site quickstart), the destination documentation was repaired first so those pointers do not lead to a dead end, and a real defect surfaced on the way, since the docs twice called the state file "gitignored" when that is a property of this repository's `.gitignore` and not of the file, meaning anyone following them would have committed a file holding their initiative, decisions, and artifact paths. The worked example is a published sample pair on the Storevine thread, an interview synthesis and the PRD that consumes it, readable end to end with a walkthrough in the concepts documentation; its stated limit is that it demonstrates the loop as designed rather than as executed. The AI-product skill family opens with four conditional sections across three skills: `deliver-prd` 3.0.0 gains `AI Behavior and Evaluation`, pairing each behavior requirement with the evidence that holds it and giving refusal and abstention their own rows, plus `Agent Execution Contract` declaring authoritative sources, do-not-touch boundaries, a requirement-to-verification map and a stop-and-escalate rule; `measure-instrumentation-spec` 3.0.0 gains `Model Trace Capture`, a privacy contract covering capture, minimization before egress and again before durable storage, failure behavior at both, terminal disposition of a failed trace, who can read a trace, whether that read is logged, retention, sampling and opt-out; `develop-adr` 3.0.0 gains `Model Choice` inside Consequences. Those three are skill-MAJORs rather than the minors first planned, a correction made during the release: conditionality narrows who is affected without changing what happens to them, the tie-breaker asks whether existing usage breaks, and it did, since two published PRD samples describing an AI-generated summary feature were non-compliant with the skill that produced them until both were retrofitted. Eight skills bumped in total, three major, four minor, one patch. Samples grow to 213 across 63 skills. No new skills; catalog stays 68 skills (30 phase + 11 foundation + 12 utility + 15 tool), 6 sub-agents unchanged. Additive MINOR.
...
- **Project memory (opt-in).** A `schema: 1` state file at `.claude/pm-skills.local.md` carrying `phase`, `active_initiative`, an `artifacts[]` ledger, and a `## Decisions` section, under a four-tag provenance model (`observation` / `interpretation` / `hypothesis` / `decision`). The `SessionStart` phase router reads a declared phase and states it rather than hedging; `memory_auto_append` is an opt-in flag that fails closed. Fully documented in [Hooks and project memory](https://product-on-purpose.github.io/pm-skills/concepts/hooks/), including the `artifacts[]` entry shape, because nothing normalizes the file at runtime: two skills inventing different shapes could not hand off, which is the whole point. Each of the six writing contracts carries a **Write discipline** bullet requiring a re-read immediately before writing and a merge rather than an overwrite, which the validator enforces as a declaration; the same documentation states plainly that this is an instruction to the agent and not a runtime guarantee.
- **`## Project Memory Contract` on eight skills:** `discover-interview-synthesis`, `deliver-prd`, `foundation-okr-writer`, `iterate-retrospective`, and all four `foundation-meeting-*` skills, each declaring what it reads, what it appends, and under which posture. Two meeting skills are pure readers by design, since that family already chains artifacts by filename and memory carries durable context instead. Each skill takes a MINOR bump with a HISTORY row.
- **Trigger-eval fixture packs for the 10 remaining `utility-*` skills**, 20 queries each, lifting the fixture-covered roster to 53.
- **`excluded:` in `trigger-eval-roster.yaml`** plus an `EXCLUDED` loader export, recording as data the decision to rule the 15 `tool-*` sprint stages out of the roster by design, with the counter-argument and reversal path written alongside it. Sprint stages are entered through a family or workflow entry point, so measuring free-text routing to them measures a path users do not take.
- **`scripts/check-memory-contracts.mjs`** (advisory): structural validation of the memory declaration, clean at 8 of 8.
- **Internal-doc link scanning** via `check-root-doc-links.mjs --include-internal` (advisory).
- **Regen-on-release-PR** (`.github/workflows/release-please-regen.yml`): a push-triggered workflow on release-please's own branch that regenerates every derived surface and commits the mirrors, closing the stale-surfaces failure that sank the bot's PR #229. It fired for the first time during this release and proved idempotent: it committed the mirrors, its own push re-triggered it, and the follow-up run found nothing to change.
- **`gen-derived-surfaces --about`**: the GitHub About string becomes generator-derived, making the post-tag About-sync step safe on its first fire and reducing the runbook's manual About step to a one-liner that cannot drift from it.
- **X-2 (artifact schemas) filed as a tracking issue** ([#258](https://github.com/product-on-purpose/pm-skills/issues/258)) with a thin effort brief. Nothing is built, but the staged typed-handoff envelope ([#224](https://github.com/product-on-purpose/pm-skills/issues/224)) stops being trigger-unreachable.
- **A tracked Phase-0 scope spec for the AI-product skill family**, replacing a maintainer-local five-name sketch no external reader could see. Four proposed skills with phases, four increments to skills that already exist, seven exclusions with reasons, and a new build gate: a proposed skill must beat a no-skill control arm before it is built, not after. Nothing in that family is built by this release.

### Changed

- The `SessionStart` phase router prefers a declared phase over its branch-name and artifact heuristics.
- Three collision pairs declared (22 to 25), with reciprocal boundary pointers on `utility-pm-workflow-orchestrator`, `utility-pm-skill-validate`, and `utility-pm-skill-auditor`. Trigger-eval execution batches extended with three wave-2 batches, collision-critical first.
- `CONTRIBUTING.md` now documents two conventions that previously existed only in enforcement: the roster rule (a new skill is rostered or `excluded:` with a rationale, in the merge that adds it) and the `## Project Memory Contract`, whose shape an advisory validator had been checking without the contributor guide ever mentioning it.
- Routine dependency updates landing in this tag: `@astrojs/starlight` 0.41.6 to 0.41.7 and `astro` 7.1.6 to 7.2.0 (#264), and `github/codeql-action` 4.37.4 to 4.37.6 (#263).
- **Plugin manifest descriptions trimmed to a current-version summary (#265).** The three generator-tracked description fields render as the marketplace tile copy in every directory that lists pm-skills, and had accumulated the narration of 19 consecutive releases reaching back to v2.17.0, so a directory visitor read about SIGPIPE races, Dependabot alerts, and Starlight draft-field shadowing instead of what the plugin does. Each field now carries its headline plus one sentence on the current version and links to this file for the rest: `.claude-plugin/plugin.json` `$.description` 9,382 to 606 characters, `.claude-plugin/marketplace.json` `$.plugins[0].description` 7,518 to 543, and `.codex-plugin/plugin.json` `$.interface.longDescription` 1,534 to 620. This does not reverse v2.31.0's OQ-3 ruling that the narration tail stays hand-maintained: the tail is still authored by hand, now from a one-sentence base rather than an append-only one. The generator-owned count headlines are byte-identical at 298, 235, and 312 characters, so `gen-derived-surfaces --check` reports every derived surface current and no count-bearing text moved.

### Fixed

- **The documented project-memory config did not parse.** The frontmatter reader did not strip trailing YAML comments, and the example published in the hooks documentation carries them, so anyone who copied it verbatim got no declared phase and no active initiative, silently, because that reader fails open by design. Comment handling is now quote- and list-aware, so `issue#42`, `"Sprint #14 cleanup"`, and `[a, b] # note` are all preserved. The regression test parses the example out of the real documentation file rather than a copy of it, which is the gap that let this ship: every prior test wrote its own input, so the parser was only ever exercised against what its author wrote, never against what a user copies.
- **The trigger-coverage completeness claim is now actually asserted.** The suite checked `ROSTER.length === 53` with the 53 + 15 = 68 accounting written only in a comment; `EXCLUDED` was exported but imported nowhere and `skill-manifest.json` was never consulted, so a new skill added to neither list would have left CI green while the published claim quietly became false. Now asserted as exact set equality between the catalog and the disjoint union of roster and exclusions, in both directions, with every excluded name required to be tool-classified so the exclusion cannot become a dumping ground.
- **The marketplace release pin.** `plugins[0].version` and `plugins[0].source.ref` now advance with the version (v-prefixed) without re-serializing `marketplace.json`, replacing the release-please jsonpath entry that corrupted the file's formatting and could not write the ref at all.
- **Two published reference pages that stated this release's own facts wrong.** The evals page carried "43 of 68 (about 63%)" for trigger coverage, the number this release replaces, and now states the accounting with a section on why coverage here is closed rather than a percentage climbing toward 100. The runtime-components page described `.claude/pm-skills.local.md` as "deferred to v2.17+", a file live since v2.25.0 and now the project-memory substrate.
- Two broken relative links in the internal docs that no guard could see, one long-standing; a HISTORY template that carried the em-dash scar it documented; a pre-existing one-sided boundary pointer between `utility-pm-workflow-builder` and `utility-pm-workflow-orchestrator`; and the missing `HISTORY.md` for `foundation-okr-writer`, with its 1.0.0 row backfilled.

### Security

- Dependency alert queue drained to zero, twice in one cycle: the standing six moderate and low alerts, then four highs published hours later. `@astrojs/starlight`, `github/codeql-action`, `mermaid`, `dompurify`, and `js-yaml` bumps (#254, #255, #256, #259, #260, #261), plus an authored lockfile-only fix for `js-yaml` 3.15.1 in `.github/scripts` and `nanoid` 3.3.18 in the site (#262) for which no Dependabot PR existed. Ten alerts closed; queue verified at zero through the API.
...
- `scripts/gen-derived-surfaces.mjs` (724 lines) + `scripts/gen-derived-surfaces.test.mjs` (44 tests): the zero-drift generator (M-36, WS-Z2/WS-Z3). Follows the `gen-skill-manifest.mjs` pattern (EOL-agnostic `--check`, loud marker refusal on a missing pair) and reads `skill-manifest.json` as its one source of truth. Emit targets: the README `pmskills:catalog-badges` / `pmskills:catalog-table` regions; the shared quickstart fragment (`scripts/data/quickstart-fragment.md`) rendered onto both `QUICKSTART.md` and the site quickstart; the sub-agent compatibility matrix (from `skill-manifest.json` cross-joined with the new `scripts/data/sub-agent-compat.json`); the three manifest description headlines (counts only; the authored per-version narration tail is untouched, OQ-3 resolved as "keep the tail hand-maintained"); and three release-notes mirrors sourced from `CHANGELOG.md` (the README `pmskills:latest-release` block, the site `changelog.md` top mirror, and the `releases/index.md` top rows, the last of which reads each release page's own frontmatter `description` as its highlight).
- `release-please-config.json` + `.release-please-manifest.json` (seeded at 2.30.0) + `.github/workflows/release-please.yml` (M-21, issue #136, WS-Z1): manifest mode, `release-type: simple`, `include-component-in-tag: false` (keeps the existing `vX.Y.Z` tag format), `extra-files` updaters for the three plugin manifests plus `x-release-please-version` annotations on the README badge and Current-version row (both sit outside every `pmskills:*` marker so the two systems never own the same line). Ships **in shadow only**: the workflow triggers on push to `main` and opens a Release PR that is observed, never merged, this cycle; see `docs/internal/release-plans/v2.31.0/release-please-primer.md` for the mechanics and the cutover checklist for what has to be true before a later tag can promote it to authoritative.
- `scripts/check-pr-title.mjs` (+ tests) and `.github/workflows/pr-title-lint.yml` (ZD-3): a Conventional Commits PR-title lint, advisory (`continue-on-error`), because squash-merge titles become load-bearing the moment release-please derives a version bump from them.
- Trigger fixtures for 12 more skills, lifting the roster from 31 to 43 of 68 (about 63%): `define-prioritization-framework`, `discover-journey-map`, `discover-market-sizing`, `foundation-meeting-agenda`, `foundation-meeting-brief`, `foundation-meeting-synthesize`, `foundation-prioritized-action-plan`, `foundation-stakeholder-update`, `foundation-lean-canvas`, `measure-survey-analysis`, `utility-pm-changelog-curator`, `utility-mermaid-diagrams`. Each carries 18 labeled queries (a 60/40 split) with near-miss negatives at a declared partner; 16 new collision pairs are registered alongside them in `scripts/trigger-eval-roster.yaml`.
- `.github/workflows/output-eval.yml` (WS-Z5): `workflow_dispatch` plus a monthly cron, `dry_run` true by default (runs the output-eval unit suite and the deterministic asset gate, no API calls); the live leg is gated on `secrets.ANTHROPIC_API_KEY` and, when present, runs exactly one roster skill as a shape-verification probe.
- `site/src/content/docs/reference/evals.md`: a published evals page naming the three lanes (trigger fixtures, the controlled router eval, LLM-judged output evals), the environment-confound lesson behind why the router eval is the trustworthy instrument, and current counts (43/68 trigger fixtures, 12/68 output-eval scenarios) with no fabricated aggregate score.
- `site/src/content/docs/reference/provenance.md` (WS-Z6): a provenance/trust page restating what the plugin ships, that nothing executes at install time, the hook opt-in model, and the supply-chain posture, for readers who never open the repo.
- Tracking issues filed for the four staged workstreams held for later releases (none built here): memory / artifact ledger (#223), typed handoff envelope (#224), coverage-offense C3 AI-product family (#225), eval-completion tail (#226).

### Changed

- `README.md` converted in place (ZD-2 = A: keep the full document, no slimming): the Skill Library count badges and the Classification table now live inside `pmskills:catalog-badges` / `pmskills:catalog-table` markers; the ~250-line per-version "Recent Updates" history and the second "Quick Release History" table collapse into one generated `pmskills:latest-release` mirror (net -261 lines). The version badge and the "Current version" row are deliberately left outside every marker, annotated `x-release-please-version` instead, so release-please and the generator never contend for the same line.
- `QUICKSTART.md` and the site quickstart now render from the one shared `scripts/data/quickstart-fragment.md` fragment instead of two hand-synced copies; `site/src/content/docs/reference/sub-agent-compatibility.md` is now generated (its "as of" stamp derives from `.claude-plugin/plugin.json`'s version, so it moves with a real bump instead of needing its own edit); `site/src/content/docs/changelog.md`'s top section and `site/src/content/docs/releases/index.md`'s top rows are now generated from `CHANGELOG.md` and each release page's frontmatter.
- `check-count-consistency` and `validate-skill-family-registration` (the two awk `RSTART`/`RLENGTH` hazard scripts named in the v2.30.0 dual-shell freeze) ported from bash+PowerShell pairs to single-source `.mjs`, each verified byte-identical (or verdict-identical) to both retiring shells against the v2.30.0 fixture tree before the shells were deleted. Both rows are removed from `scripts/validation-manifest.yaml` (23 dual-shell pairs remain) and their four per-shell `validation.yml` steps collapse to two cross-OS `node` steps; `scripts/check-validator-parity.mjs` updated for the reduced shell set.
- `SECURITY.md` (root and its site mirror) expanded: what the plugin ships, that skills and docs are inert markdown with nothing executing at install, the hook opt-in model (`.claude/pm-skills.local.md`, fail-open), and the supply-chain posture (SHA-pinned `agent-plugins` marketplace, a per-zip SHA-256 checksum, the scoped release-please token). Linked from `README.md` and `getting-started/index.md`.
- The three plugin manifests bumped to 2.31.0 with a new v2.31.0 narration sentence; `.claude-plugin/marketplace.json`'s `ref` re-pinned from `v2.30.0` to `v2.31.0` (TD-3).
- Four routine dependency bumps landed on `main` ahead of this branch and ship in this tag: `astro` 6.4.2 to 7.0.2, `@astrojs/starlight` 0.39.3 to 0.41.0, and `vite` 7.3.3 to 8.1.0 together (#213); `sharp` 0.34.5 to 0.35.1 (#206); `dompurify` 3.4.7 to 3.4.11 (#210); `js-yaml` (site) 4.1.1 to 4.3.0 (#222).

### Removed

- The `.sh` / `.ps1` / `.md` triples for both ported validators (6 files), and `scripts/shell-parity-smoke.mjs` + `.test.mjs` (its only target is now ported; the v2.30.0 fixture tree becomes the port's own acceptance corpus instead).
- README's ~250-line per-version "Recent Updates" `<details>` history and the second "Quick Release History" table (folded into the one generated recent-releases mirror above).

### Fixed

- Release-note duplication (P1-6): six per-release note locations reduce to two still-authored surfaces (the CHANGELOG enrichment and the per-release site page) plus three now-generated mirrors (the README recent-releases block, the site changelog top mirror, and the releases-index row); every existing per-release URL keeps resolving (route-parity green).
