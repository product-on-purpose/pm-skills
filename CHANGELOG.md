...
- **4 dispatch skills shipped with Codex-scoped validation** per master plan D30 Option A. **What was validated:** Codex CLI 0.128.0 on 2026-05-17 PASSED GATE B (3 non-conductor dispatch skills: pm-critic, pm-skill-auditor, pm-changelog-curator) + GATE C (conductor dispatch sub-spike via "reference + execute inline" chain composition in `--dry-run` mode). Context budget held through 6 gates. Layered envelope output produced consistently. Refusal protocols + no-bypass discipline worked. **What was NOT validated:** (a) actual LIVE release operations via the conductor dispatch path on Codex CLI - the GATE C test was dry-run; live G3 tag was simulated only; (b) ANY behavior on Cursor, Windsurf, Copilot CLI, or Gemini CLI - those clients are UNTESTED in v2.16.0. **Safe-usage matrix:** Claude Code native (production-ready) > Codex CLI dispatch (production-ready for non-conductor; conductor live-release use-with-caution) > other non-Claude clients (EXPERIMENTAL; per-client validation deferred to v2.17). Evidence at [`docs/internal/release-plans/v2.16.0/gate-test-results_2026-05-17_codex.md`](docs/internal/release-plans/v2.16.0/gate-test-results_2026-05-17_codex.md). Maintainer-driven additional-client validation can be triggered by re-running the harness at [`maintainer-gate-testing-codex.md`](docs/internal/release-plans/v2.16.0/maintainer-gate-testing-codex.md).
