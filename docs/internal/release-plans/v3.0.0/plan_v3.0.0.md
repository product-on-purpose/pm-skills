...
**Created:** 2026-05-25, when the v3 line was reframed. The additive marketplace launch became [v2.21.0](../v2.21.0/plan_v2.21.0.md) (minor) and the naming standardization became [v2.22.0](../v2.22.0/plan_v2.22.0.md) (minor, hard rename). `v3.0.0` is reserved for the one release that actually **removes** things.
**Type:** **MAJOR.** Existing users must act for their setup to keep working: the old marketplace path goes away. (Contrast v2.21.0, which removed nothing; v2.22.0 removes the old skill names but ships as a minor under the recorded stance that skill invocation names are outside the SemVer-governed surface. This major is the marketplace-path removal, a change to the governed install contract.)
...
Retire the old marketplace path in a single, well-signaled major, at the moment a second plugin makes the new marketplace clearly worth moving to. (The skill-name alias removal that this release originally also carried is gone: v2.22.0 did a hard rename, so there are no aliases left to remove.) Aimed at the *removal*, not the additive launches that preceded it.
...
The skill-name alias removal that earlier drafts bundled here is **no longer applicable**: v2.22.0 performed a hard rename (D-V31-4=A) with a migration table instead of a deprecation-alias window, so the old phase-prefixed names are already gone. No aliases remain to remove.
...
- v2.22.0 (naming) SHIPPED. (No alias window to wait out; the hard rename completed the naming change in v2.22.0.)
...
- Sweep any remaining references to the old marketplace path; correct docs. (No name-alias removal: v2.22.0's hard rename already removed the old skill names.)
...
- [`../v2.22.0/plan_v2.22.0.md`](../v2.22.0/plan_v2.22.0.md) - the naming standardization (hard rename; D-V31-4=A, so no alias removal is deferred here).
