...
**Source:** Strategic roadmap R-11 (2-source consensus: Codex backlog + Claude Sonnet backlog; web research confirms PM-domain importance)

This document is a full SKILL.md draft for review and refinement at v2.18.0 execution time. At execution, the maintainer copies this draft to `skills/measure-survey-analysis/SKILL.md` and authors the companion TEMPLATE.md + EXAMPLE.md + 3 thread-aligned samples + slash command file.
...
- Inputs to this skill often come from: `discover-research-plan` (the survey design), `discover-survey-design` (when shipped; the question-writing skill), live survey results
...
- Original spec: `docs/internal/release-plans/v2.18.0/spec_measure-survey-analysis.md`
- Roadmap source: R-11 in `docs/internal/_working/roadmap_opus-4.7-max_2026-05-14.md` Section 6
- Companion command: `commands/measure-survey-analysis.md`
- TEMPLATE: `skills/measure-survey-analysis/TEMPLATE.md`
- Examples: `skills/measure-survey-analysis/EXAMPLE.md` + library samples in `library/skill-output-samples/measure-survey-analysis/`
```
...
### `library/skill-output-samples/measure-survey-analysis/brainshelf.md`

Brainshelf scenario: NPS survey from 1200 Brainshelf subscribers asking satisfaction + "what would you change?" Open-text responses cluster into 5 themes. Analysis demonstrates how to use NPS as tracking signal (vs. as the diagnostic) and how to mine open-text for prioritized feature recommendations.

Target output: ~200-250 lines. Demonstrates tracking analysis pattern + open-text thematic clustering. Specifically shows: NPS distribution with previous-period comparison; 5 themes from open-text with rep quotes and mention counts; 4 prioritized recommendations with confidence labels; explicit "what NPS does not tell us" section.

### `library/skill-output-samples/measure-survey-analysis/storevine.md`

Storevine scenario: B2B feature-prioritization survey from 180 customer admins. Survey designed to validate which forecasting feature to build next. Analysis demonstrates hypothesis validation pattern with strong sample but mixed signal.

Target output: ~200-250 lines. Demonstrates validating-single-hypothesis pattern. Specifically shows: per-question response distribution with confidence intervals; segmented breakdown by company size (n=180 splits to n=45/60/75 across 3 size buckets, with caveats on smaller segment); 1 supported + 1 contradicted hypothesis; recommendation with counter-evidence.

### `library/skill-output-samples/measure-survey-analysis/workbench.md`

Workbench scenario: Internal developer-experience pulse survey from 65 engineers. Sample too small for strong statistical inference but useful for direction. Analysis demonstrates honesty about limitations.

Target output: ~200-250 lines. Demonstrates exploratory analysis pattern with weak sample. Specifically shows: response distribution flagged as direction-only; thematic clustering of open-text (5 themes); explicit refusal to make capital-allocation recommendations; recommendation for follow-up dev-experience-dedicated quarterly survey with n=300 target.
...
- [ ] `metadata.classification: phase`, `metadata.phase: measure`
- [ ] Description under 1024 chars
- [ ] Refusal protocols section lists 6+ scenarios
- [ ] Output format has 9 named sections
- [ ] Patterns section documents 4 patterns (single-hypothesis / exploratory / segmented / tracking)
- [ ] Cross-skill composition identifies upstream (research-plan), downstream (problem-statement, prd), and qualitative complement (interview-synthesis)
- [ ] TEMPLATE.md scaffold matches the SKILL.md output structure
- [ ] EXAMPLE.md walks through a complete worked example (recommend single-hypothesis validation pattern)
- [ ] 3 thread-aligned samples covering 3 patterns (tracking / single-hypothesis / exploratory)
...
|---|---|---|---|
| Skill produces over-confident statistical claims | High | High | Refusal protocols #1 and #2 explicitly catch insufficient-sample and leading-question; description and Core Principle emphasize honesty about limitations |
| Sample fabrication: hard to write realistic survey data without real survey results | High | Medium | Each sample documents the "as if" survey design + response distribution; flag samples as illustrative |
| Confusion with `measure-experiment-results` | Medium | Low | Survey analysis is observational; experiment results is causal. Cross-reference both in description. |
| User asks for causal inference from cross-sectional survey | Medium | High | Refusal protocol #5 explicitly catches this; recommends switching to experiment-design |
| Overlap with `discover-interview-synthesis` | Low | Low | Survey analysis is quantitative; interview synthesis is qualitative. Pair them; they're complements not substitutes. |
| NPS-only decision input | Medium | Medium | Refusal protocol #4 catches; description mentions NPS as tracking-not-diagnostic |
...
- Parent plan: [`plan_v2.18.0.md`](plan_v2.18.0.md)
- Strategic roadmap R-11: `../../_working/roadmap_opus-4.7-max_2026-05-14.md` Section 6
- Companion specs:
