...
> - **Remaining (Phase 5, maintainer-run):** go-public hardening (README de-Preview [done in-repo] + branch protection) -> flip agent-plugins public -> **S8** (post-flip public install from a non-org-authenticated client; the one untested-against-public scenario) -> publish GitHub Release -> apply M1 repoint (`m1-repoint.md`) -> announce -> post-tag hygiene.
