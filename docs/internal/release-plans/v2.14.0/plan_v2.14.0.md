...
| Post-tag follow-ups (FU1-FU8 + M1-M3 + V1-V11) | All deferred-to-v2.14.x items shipped to main 2026-05-10 (not tagged; awaiting v2.14.1 or v2.15). ~20 commits. Two validators promoted from advisory-in-name-only to truly enforcing (check-internal-link-validity via FU6; validate-docs-frontmatter via V5). Two long-standing failing workflows closed (sync-agents-md disabled per V7; validate-mcp-sync flipped to observe per V9 + cross-repo update to pm-skills-mcp/pm-skills-source.json). Mermaid 3-layer beautification + style guide (MD + HTML) shipped. v2.14.x backlog table is empty; next session opens v2.15+ scoping. See AGENTS/claude/CONTEXT.md status block + CHANGELOG `[Unreleased]` section for the current state. |
