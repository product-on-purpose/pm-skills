...
| `pm-skills-mcp` cross-repo surfaces | **Nothing. Tick N/A and move on.** | **Frozen by the 2026-05-04 maintenance-mode ruling. No per-release action, whatever this release changes about the catalog or the narrative.** The condition that used to sit here ("only if the release changes the catalog narrative or skill counts") was unanswerable in practice: v2.33.0 met it and the correct action was still nothing, so it cost a judgement call every cycle and produced none. Revisit only if O-15 (pm-skills-mcp sunset review), in `agent-config/OPEN.md`, rules resume. **Do not send readers to internal runbook 10.5.5**, whose surrounding paragraph also asserts the MCP is "frozen at v2.9.2" when it is v2.9.3; that whole document is a stale predecessor, tracked under [#269](https://github.com/product-on-purpose/pm-skills/issues/269) | |
