...
| WS-2 | C-1 B1 keystone build: `.claude/pm-skills.local.md` schema:1 + ledger delta, F-44 router reads it, propose-then-confirm write path + opt-in auto mode, docs + opt-in posture preserved (inert when absent) | WS-1 | claude | B1 live behind opt-in; validators green; router honors the file |
