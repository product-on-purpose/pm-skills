...
| C-5 Validator coverage | PARTIAL (2026-06-14): `scripts/check-reciprocal-boundary-pointers.mjs` (+ 8-test `.test.mjs`) asserts every declared `COLLISION_PAIR` points at the other in "When NOT to Use". Wired ADVISORY in validation.yml (M-30 ladder). It found ONE real gap: `foundation-meeting-recap` does not point back to `discover-interview-synthesis` (the reverse exists). PROMOTE to enforcing once that back-pointer + a foundation-meeting-recap version bump land. STILL PENDING: also fold eval-asset-presence (B-4) + this reciprocity check into the `utility-pm-skill-validate` skill's own report. |
