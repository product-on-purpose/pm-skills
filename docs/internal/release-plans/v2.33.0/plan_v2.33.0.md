...
**Status:** **SHIPPED 2026-09-01.** Tag `v2.33.0` pushed 2026-09-01, annotated, pointing at `7a42570e`. Both tag-triggered workflows green (Release, Validate Plugin Packaging) and the GitHub Release published with its three assets. **G0 through G4 CLOSED 2026-09-02.** G4 ran across two days: the same-day doc work, then the delivery repair the following day after a field report showed the release was undeliverable. Its **P0 install smoke test PASSED** against the published artifact through the real user path. The one section C row still open, skills.sh, is deferred by its own "after a delay" condition rather than outstanding. **The IN PROGRESS wording this replaces was correct when written**, and is recorded rather than overwritten silently, because an earlier draft had claimed G4 closed while three rows sat open. **This distinction is deliberate.** An earlier draft of this block read "G3 and G4 closed" while the section C table in the same commit marked three rows open, which is the pre-satisfied-control defect R11-F3 and R12-F3 both caught in this cycle. Corrected on the same day it was introduced. The cut was paused at READY TO TAG from 2026-08-29 to 2026-09-01 by maintainer choice.
...
**Gate G3 CLOSED 2026-09-01; gate G4 IN PROGRESS.** G3 tagged the G2.5-captured SHA and no other, per the invariant. **The tag-target question was ruled at G3 against the wrap log's recommendation:** the log proposed HEAD (`605edcaa`), and the evidence went the other way. Both the runbook and this plan name `7a42570e` as the only legal target; all five workflows ran green directly on `7a42570e` while `Validate Plugin Packaging` never ran on `605edcaa` (skipped by the push `paths:` filter, since that commit touches only this doc); and tagging `605edcaa` would have embedded, inside the tagged tree, a plan doc naming a different SHA as the only legal target. G4 then closed both halves of the release-plans index row, re-authored the three manifest description tails (a G2 miss: G2 moved the marketplace release pin but not the authored pitch beside it), and replaced the auto-published Release body. **What G4 completed on 2026-09-02.** The **P0 plugin-install smoke test PASSED** against the published artifact, not the local tree: `marketplace update` then `install pm-skills@product-on-purpose` returned 2.33.0 at `7a42570e` carrying 68 skills / 6 sub-agents / 11 commands, all three skill-MAJORs at 3.0.0, and all four new AI sections present. The smoke install was scoped local to a scratch directory and uninstalled, leaving the two pre-existing 2.24.0 project scopes untouched. Note `validate-plugin-install` in the pre-tag bundle exercises the local tree and does **not** satisfy this. The `agent-plugins` re-pin merged; the `pm-skills-mcp` row closed N/A by standing policy with its review re-homed to a dated one. Only skills.sh remains, deferred by its own condition. **Release complete.**
...
| `pm-skills-mcp` surfaces | **Nothing. N/A by standing policy** | [x] **CLOSED N/A 2026-09-02.** **Frozen by the 2026-05-04 maintenance-mode ruling. No per-release action, whatever this release changes about the catalog or the narrative.** The condition that used to sit here ("only if the release changes the catalog narrative or skill counts") was unanswerable in practice: v2.33.0 met it and the correct action was still nothing, so it cost a judgement call every cycle and produced none. Revisit only if O-15 (pm-skills-mcp sunset review), in `agent-config/OPEN.md`, rules resume. Ruled the same day: keep maintenance mode, add the dated review. Counts half was N/A regardless, catalog holds at 68 / 6 |
