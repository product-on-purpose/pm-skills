...
**Gates G3 and G4, closed 2026-09-01.** G3 tagged the G2.5-captured SHA and no other, per the invariant. **The tag-target question was ruled at G3 against the wrap log's recommendation:** the log proposed HEAD (`605edcaa`), and the evidence went the other way. Both the runbook and this plan name `7a42570e` as the only legal target; all five workflows ran green directly on `7a42570e` while `Validate Plugin Packaging` never ran on `605edcaa` (skipped by the push `paths:` filter, since that commit touches only this doc); and tagging `605edcaa` would have embedded, inside the tagged tree, a plan doc naming a different SHA as the only legal target. G4 then closed both halves of the release-plans index row and the section C external surfaces.
...
| `agent-plugins` re-pin | Every release. Complete that repo's Section 7 checklist in the PR body | [ ] **OPEN 2026-09-01.** Cross-repo, needs a PR against `agent-plugins`. Nothing here blocks it |
