...
After install, all 60 skills resolve from any directory. Slash commands like `/prd`, `/opportunity-tree`, `/okr-writer`, `/agenda` work immediately. Verify with `/plugin list`.
...
Installs all 60 skills into Claude Code's default skills directory. No clone, no sync. Works identically across most agents (see [Cursor](#cursor), [Copilot](#github-copilot), [Cline](#vs-code-cline--continue)).
...
The ZIP contains all 60 skills, slash commands, workflows, library samples, and a `QUICKSTART.md` with detailed instructions.
...
Open Cursor with the workspace that includes `pm-skills`. Cursor's AI assistant will discover all 60 skills via `AGENTS.md`. Invoke by name:
...
Open the folder in Windsurf. The AI assistant discovers and can invoke all 60 skills. No sync helper needed; Windsurf reads `AGENTS.md` directly.
