...
!!! warning "MCP server is in maintenance mode"
    ...
    The companion MCP server [`pm-skills-mcp`](https://github.com/product-on-purpose/pm-skills-mcp) entered maintenance mode on 2026-05-04. The published [v2.9.1](https://github.com/product-on-purpose/pm-skills-mcp/releases/tag/v2.9.1) release remains available on npm and continues to expose 29 PM skills, 11 workflow tools, and 8 utility tools via the Model Context Protocol. Active development is paused pending demonstrated demand. Security patches and critical bug fixes will continue; new skill parity with this `pm-skills` library is on hold.
...
If your team has already adopted `pm-skills-mcp` or has a specific reason to use the MCP transport (e.g., a client that does not support file-based skills), the v2.9.1 release remains fully functional. Setup, configuration, and troubleshooting are documented in the MCP server's own README:
...
| | This `pm-skills` library | `pm-skills-mcp` v2.9.1 (frozen) |
|---|---|---|
| Total skills | 40 | 29 |
| Workflows | 9 | 11 (legacy names retained) |
| Active maintenance | Yes | Maintenance mode |

Skills added to `pm-skills` after the v2.9.1 build of `pm-skills-mcp` are not embedded in the MCP server. Use the file-based install for current parity.
