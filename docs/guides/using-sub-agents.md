...
|---|---|---|
| `pm-critic` | `/critic [optional path]` | After producing a PM artifact (PRD, OKR set, persona, etc.) - get adversarial review with P0/P1/P2/P3 findings |
| `pm-skill-auditor` | `/audit-repo [--scope] [--severity-floor]` | Pre-release governance audit or repo-health check |
| `pm-changelog-curator` | `/draft-changelog [--since-tag] [--target-version]` | Release prep: draft CHANGELOG from git log |
| `pm-release-conductor` | `/release v{X.Y.Z} [--dry-run]` | Walk the full 6-gate release runbook |
...
```
/critic                            # Reviews the most recent artifact in session context
/critic docs/specs/my-prd.md       # Reviews the specified artifact
/audit-repo                        # Runs the full repo audit
/audit-repo --scope changed        # Audit only working-tree-changed files
/draft-changelog                   # Drafts entries since the most recent tag
/draft-changelog --since-tag v2.15.0 --target-version v2.16.0
/release v2.16.0 --dry-run         # Rehearse the release flow
/release v2.16.0                   # Live release
```
...
The conductor (`pm-release-conductor`) does NOT support @-mention because release operations are too consequential for ambient spawning. Explicit `/release v{X.Y.Z}` is required.
...
```
/release v2.16.0           # Live
/release v2.16.0 --dry-run # Rehearse without tag + push
```
...
```
You: /audit-repo --severity-floor P1
...
```
/draft-changelog --committed-only --since-tag v2.15.0
```

### Problem: pm-release-conductor fails G0

**Solution:** fix the underlying issue and re-invoke `/release`. The conductor refuses bypass; G0 is idempotent (re-runs from scratch on each invocation).
