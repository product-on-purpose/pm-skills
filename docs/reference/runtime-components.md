...
| `pm-release-conductor` | Maintainer | Explicit only: `/pm-release v{X.Y.Z}` (no @-mention; release too consequential for ambient spawn) | Full release flow (often 30+ minutes) | Bash, Read, Edit, Grep, Glob, **Agent** (chain-permitted per `_chain-permitted.yaml`) | Chains to `pm-skill-auditor` at G0 + G2.5 and to `pm-changelog-curator` at G2; reads `docs/contributing/release-runbook.md` for gate definitions | `skills/utility-pm-release-conductor/` |
