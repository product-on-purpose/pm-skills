...
- Re-invoke `/release v2.16.0` once resolved
...
3. **G0 idempotency.** Re-invoking `/release` re-runs G0 from the top. The conductor does not cache prior sub-check results; each invocation is a fresh full audit.
