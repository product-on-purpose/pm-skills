...
Best retrieval score of the three at 0.83 top-10 recall [fictional]. Rejected because 2 percentage points of recall did not justify roughly 3x the per-token cost and onboarding a second vendor, including a new data-processing agreement. The 2-point gap is **unresolved rather than dismissed**: the point estimate does not establish a reliable or material advantage, because two point estimates and a set size say nothing about the uncertainty around the difference and no per-query paired analysis was run to find out. It is directional evidence of unknown weight, not a result to lean on and not one to wave away. The decision below is provisional on that: it rests on the cost and vendor arguments, which do not depend on resolving the gap. Reproducing this number requires the asymmetric `input_type` setting Cohere v3 expects, `search_query` for the query side and `search_document` for stored items; scoring both sides as documents changes retrieval and would not reproduce the 0.83.
