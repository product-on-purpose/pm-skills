...
status: sample
thread: orbit
context: Orbit async workspace . Orbit Digest PRD
---

# Sample: Orbit Digest . Product Requirements Document
*Skill:* `deliver-prd` | *Thread:* Orbit | *Sample:* 01 of 03
...
Team context:
Orbit is a B2B async workspace, Series B, ~120 employees. We're expanding from SMB
to mid-market enterprise. Product team is 3 PMs + 1 design researcher.

Discovery finding:
Enterprise teams create Orbit channels but only 17% of updates are watched within
48 hours [fictional]. Root cause: no shared context before video plays . recipients
can't decide if a 4-minute update is worth opening. This is killing our enterprise
...
Success targets:
- Async update open rate: 17% → 30% within 90 days of GA [fictional]
- Enterprise trial-to-paid conversion: 19% → 26% within 6 months [fictional]
- Digest adoption: 60% of enterprise workspaces enabling Digest on ≥1 channel,
  30 days post-GA [fictional]

Scope for this release:
- Auto-generate summary for all new async updates on Digest-enabled channels
- Display summary inline in thread view, above the video player
- Creator controls: regenerate, edit, hide summary on a specific update
- Team-level default: Digest on/off per channel (admin setting)
- Workspace-level default: on for Enterprise plan, off for Starter/Free
- NOT in scope: retroactive summarization, live meetings, public channel summaries,
  AI confidence scores shown to end users

Technical decisions already locked:
- HLS streaming (not WebRTC) . ADR-2026-03
- Whisper (self-hosted) for transcription; Deepgram and AssemblyAI ruled out in spike
- Summary stored as metadata on the video record, no separate table
- No confidence score in any user-facing surface (shows confidence indicators
  reduce trust rather than build it)

Stakeholders: Maya Okonkwo (Engineering Lead), Priya Nair (Design Lead),
Diego Flores (Enterprise Sales), Isabel Chen (Legal/Privacy)
```
...
Orbit Digest's output is generated by a model, so "the summary is good" is a claim that needs
evidence attached. Each behavior below is a release requirement, not a stretch goal.

### Behavior Requirements and Their Evidence

| ID | Behavior required | How it is evaluated | Passing threshold |
|----|-------------------|---------------------|-------------------|
| AB-1 | Every claim in the summary is supported by the transcript. No invented names, dates, decisions, or commitments | Human-labelled faithfulness pass over held-out transcript and summary pairs; a summary fails if any sentence asserts something the transcript does not | 0 unsupported claims on the harmful slice; 98% [fictional] of summaries fully supported overall. Chosen because a fabricated commitment in a work update is the failure that loses trust in the feature, and one is too many |
| AB-2 | Refuses to summarise when the transcript carries no substantive speech, emitting the FR-4 "Summary unavailable" state rather than a plausible-sounding filler summary | Silent, music-only, and ambient-noise recordings from the edge-case set | 100% [fictional] emit the unavailable state. A filler summary here is worse than no summary, so this threshold does not trade off |
| AB-3 | Abstains when transcription confidence is below the usable floor, rather than summarising a garbled transcript | Low-confidence transcripts (heavy accent, poor microphone, crosstalk) scored against whether the summary was emitted or withheld | 95% [fictional] withhold. The 5% tolerance is for borderline confidence, and misses surface as AB-1 failures rather than silently |
| AB-4 | Summarises in the transcript's detected language and labels the Digest accordingly, per the non-English edge case | Non-English transcripts across the languages present in the pilot workspaces | 97% [fictional] correct language and label. Below that, the label is more misleading than absent |

Scored separately rather than end to end: transcription and summarisation are distinct steps, and a
combined pass rate would hide which one failed. AB-1 is scored only on transcripts that AB-3 did not
withhold, so abstention is not rewarded as faithfulness.

### Evaluation Set

- **Where the cases live:** `orbit-eval/digest/` in the analytics repo, versioned alongside the
  summarisation service
- **Slices that must be covered:** every AB-n row above; the refusal case (AB-2) and the abstention
  case (AB-3) as their own slices; and the input classes where a failure costs the most, namely
