...
**Status:** **v2.17.0 SHIPPED 2026-05-20** via `/pm-release v2.17.0` (6-gate conductor run). Annotated tag `v2.17.0` at commit `b6ff373`; GitHub Release published with the rich Release_v2.17.0.md body; tag CI green (Validate Plugin Packaging + Release workflow). Delivered: native Claude Code sub-agent registration LIVE (`agents/` auto-discovery; `AGENTS/` to `_agent-context/` rename), frontmatter metadata-nested migration across 59 skills, bash-3.2 validator portability, check-version-references advisory wiring. All four work items (W1-W4) + F-P2-01 shipped. Post-tag follow-ups: merge pm-skills-mcp embed-skills.js branch; v2.18.0 (4 content skills) is the next cycle. Previous tagged release: **v2.16.2** (2026-05-19).
