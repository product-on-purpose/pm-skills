...
**Status:** **v2.29.1 SHIPPED 2026-06-24 (annotated tag `v2.29.1`, tag object `b9a6e96b`, at `9d06ff4e`; GitHub Release Latest with ZIP + sha256 + manifest; PR #215 squash-merged; tag CI green: Release + Validate Plugin Packaging; main-push Validation + Deploy + CodeQL green)** - maintenance patch: the docs-site generator (gen-site.mjs) now renders every SKILL.md section verbatim (it was silently dropping ~27 of 68 pages' method content, including foundation-build-risk-review's Hard gate / Modes / review contract / Verdict routing) behind a new enforcing output-based `check-skill-page-sections` completeness gate, plus flat skill-to-skill cross-reference link resolution in remark-resolve-links; no skill behavior change, catalog stays 68 skills / 6 sub-agents; PATCH. Previous: **v2.29.0 SHIPPED 2026-06-23 (annotated tag `v2.29.0`, tag object `3cc19bec`, at `76f793b8`; GitHub Release Latest with ZIP + sha256 + manifest; PR #212 squash-merged; tag CI green: Release + Validate Plugin Packaging; main-push Deploy + CodeQL green)** - new foundation skill `foundation-build-risk-review` (a fast pre-build / feature-scope risk gate: it names the single assumption most likely to make the work fail and returns a verdict - build small, validate first, pivot first, or do not build yet - with a no-code validation step, then routes to the next skill; two modes pre-build + feature-change; launched-product diagnosis routes to `iterate-pivot-decision`; adapted from `bin1874/before-you-build-skill` Apache-2.0, PM-neutralized) + new sub-agent `pm-skill-router` (M-34: a key-free router making the new-skill collision gate + the trigger router-eval runnable on the subscription with no API key via `check-new-skill-collision.mjs --emit-tasks`, Haiku-pinned by default) + D8 (showcase auto-derive from sample dirs + an enforcing `check-sample-counts` gate closing the library/ blind spot + retired the vestigial v2.6.1 sample manifest); catalog 67 to 68 (foundation 10 to 11), sub-agents 5 to 6; additive MINOR. Previous: **v2.28.0 SHIPPED 2026-06-20** (annotated tag `v2.28.0`, tag object `e350c5b`, at `4fb0d703`; GitHub Release Latest with rich body + ZIP/sha256/manifest; tag CI green: Release + Validate Plugin Packaging; Validation + CodeQL + Pages green on the tagged SHA; `agent-plugins` re-pinned to v2.28.0 / `4fb0d703`, registry metadata 1.28.0; two Codex adversarial reviews resolved before tag) - new foundation skill `foundation-stakeholder-briefings`, the 1-to-N audience fan-out: one source artifact (spec, discovery, research, GTM, experiment, retro, raw notes) becomes a canonical master document plus a set of audience-tailored briefings, one per stakeholder lens (executive, board, engineering, UX, PMM, sales, CS, legal, data, or custom), each a traceable projection of the master (briefing blocks carry `Draws on:` master claim IDs + one `Primary ask:`; the structural half - references resolve + one ask - is enforced by the advisory `scripts/check-briefings-trace.mjs`, full projection fidelity is the skill self-check + review). Nine first-class lenses + Custom; source-aware proposal; trigger fixtures; 18 library samples; `utility-pm-skill-builder` 1.1.0 to 1.1.1. Catalog 66 to 67 (foundation 9 to 10); additive MINOR; plan + spec in `docs/internal/release-plans/v2.28.0/`; the memory "Remember" train moved to v2.29.0. Codex adversarial review #1 (plan/spec) resolved (7 findings). Previous: **v2.27.1 SHIPPED 2026-06-16** (annotated tag `v2.27.1` at `10685b2d`; GitHub Release Latest with the rich body + ZIP/sha256/manifest assets; tag CI green: Release + Validate Plugin Packaging; maintenance PATCH: the classification sub-count drift gate in `check-count-consistency` - both shells now derive phase/foundation/utility/tool sub-counts from frontmatter and police them, closing the gap that let the 26/8/6 split drift on a published page - plus the three doc-currency fixes it surfaced (`_workflows/triple-diamond.md` 25 to 30 phase; the getting-started quickstart 65 to 66 / 10 to 12 utility / 10 to 11 commands; an anatomy-page historical mention reworded); no skill behavior change, catalog stays 66; G1 codex review waived for a deterministic validator+docs patch per the v2.27.0 "when green, push" precedent; shipped via the 6-gate runbook embodied inline; M-25 stays pending-human with a fresh `claude plugin validate` clean record, the utility-pm-critic fixture relabel deferred to v2.29.0 with the memory train). Previous: **v2.27.0 SHIPPED 2026-06-15** (annotated tag at `ee7ff9d5`; GitHub Release Latest with the rich Release_v2.27.0.md body + ZIP assets; tag CI green: Release + Validate Plugin Packaging) - the provable-quality release (additive MINOR; no new skills, catalog stays 66): trigger-accuracy evals (M-31; per-skill `evals/trigger-fixtures.json` + the router-eval instrument + a committed baseline + drift/collision CI gates, lanes proven live 2026-06-15), derived surfaces (M-32; `skill-manifest.json` + the generated AGENTS.md catalog behind enforcing staleness gates, retiring `sync-agents-md.yml`), the output-quality eval infrastructure (M-33; harness + 6 family rubrics + the tested aggregate/verdict module + a 3-arm informed control + the asset gate; per-skill RESULTS stay internal evidence), the creator/validator eval-contract integration (`utility-pm-skill-builder` + `utility-pm-skill-validate` 1.1.0 + CONTRIBUTING), and the now-enforcing reciprocity gate. Shipped via the 6-gate runbook embodied inline: G2 surface (CHANGELOG [2.27.0] + doc mirror + Release_v2.27.0.md + releases index + README badge/At-a-Glance/What's New + 3 version manifests at 2.27.0); G0 cross-cutting audit 0 P0; G1 codex adversarial review (`--base v2.26.0`) caught 2 fail-closed gaps in the NEW eval gates - the router drift gate could certify a partial run, and the single-skill output-eval omitted the absolute-failure-first verdict - BOTH fixed + unit-tested before the tag (commit `ee7ff9d5`); G2.5 SHA `ee7ff9d5` captured + CI-re-verified green both OS; G3 tag pushed; G4 GitHub Release Latest + this CONTEXT flip + the v2.28.0 stub. Previous: **v2.26.0 SHIPPED 2026-06-10** (annotated tag at `c11de12`; GitHub Release Latest with the rich Release_v2.26.0.md body + ZIP assets; tag CI green: Release + Validate Plugin Packaging; install smoke PASS: plugin updates 2.25.2 to 2.26.0 from the self-hosted marketplace) (build merged across PRs #187-#197: WS-A1 landing de-rot + per-family card enforcement, WS-A2 entry docs, WS-A3 generator-doc retirement, F-12 Batch 0 description integrity + skills/ scar sweep + guard scope, F-14 `utility-pm-workflow-builder` + F-15 `/chain` Mode B productization in one PR closing #133/#134, F-12 Batches 1-4 quality convergence closing #135 with all 26 cohort skills converged under the amended D-4, and the Task 9 orchestrator native-path smoke gate RUN + RECORDED PASS on the installed plugin: Skill-tool delegation works, downstream skills execute INLINE; procedure captured as the agentic smoke runbook. Catalog 65 to 66 skills, utility 11 to 12, commands 10 to 11; sub-agents stay 5; workflows stay 12. CHANGELOG [2.26.0] + Release_v2.26.0.md + all version manifests at 2.26.0; shipped via the 6-gate runbook embodied inline, G2.5 SHA captured and re-verified in both shells). Previous: **v2.25.2 SHIPPED 2026-06-10** (tag `v2.25.2`; GitHub Release Latest; maintenance patch resolving the remaining 2026-06-06 Codex audit items: a unified validator-inventory manifest `scripts/validation-manifest.yaml` plus an enforcing per-leg CI parity referee `scripts/check-validator-parity.mjs` that caught and reconciled a live drift (two validators enforcing in CI but absent from both local bundles), the root-doc-link checker extended to source surfaces, the em-dash-scar guard promoted to enforcing and multi-backtick aware, and the `CLAUDE.md` gitignored-claim correction; a `codex:adversarial-review` of the release ran first and its 1 P1 + 2 P2 findings were resolved before tagging; 25 referee/guard unit tests green; no new skills, catalog stays 65, sub-agents 5). Previous: **v2.25.1 SHIPPED 2026-06-06** (tag `v2.25.1` at `2b5044a`; GitHub Release Latest; squash-merged PR #175; `agent-plugins` registry re-pinned metadata 1.9.0 via PR #17; all tag + main CI green on both OS legs + CodeQL + Pages deploy; maintenance patch banking accumulated untagged work since v2.25.0: Pattern S docs-site reorg + full family-standard conformance, a generated CI-gated resource index, an em-dash-scar cleanup with new advisory + enforcing CI guards, three site dependency bumps, and a pre-tag validator parity fix reconciling the PowerShell bundle with bash + CI from the 2026-06-06 Codex audit P0-01/P0-02; no new skills, catalog stays 65; sub-agents 5). Previous: **v2.25.0 SHIPPED 2026-06-03** (tag `v2.25.0` at `23e65da`; GitHub Release Latest; squash-merged PR #161; site deployed) - The activation and trust layer: the plugin's first hooks plus an advisory output-quality CI tier; no new skills (catalog stays 65; sub-agents 5); additive minor. **F-43** opt-in `PreToolUse` house-rule guardrails (inert until `guardrails: true` in `.claude/pm-skills.local.md`; em-dash/en-dash BLOCK, placeholder/metric WARN; dependency-free Node, fail-open). **F-44** confident-only `SessionStart` phase router (suggests Triple Diamond skills only on a strong repo signal - a phase-named branch or a recognized artifact - reading `phase:` from SKILL.md frontmatter; silent otherwise). **M-30** advisory output-quality eval harness (3 deterministic invariant validators over recorded samples; no-placeholders + exact-quote-sourcing clean, no-fabricated-metrics heuristic). 43 unit tests green; full pre-tag bundle PASS; site build green (388 routes, 0 broken links/anchors); two Codex reviews (rescue + adversarial challenge) applied and findings fixed (MultiEdit/ExitPlanMode guardrail coverage, false-pass validator rewrite, artifact-ambiguity router). Post-tag verification: the live integration smoke. Previous: **v2.24.0 SHIPPED 2026-06-01** (tag `d3f1549` on main) - Adds the `pm-workflow-orchestrator` sub-agent and its `utility-pm-workflow-orchestrator` dispatch skill: a governed multi-skill runner that takes a saved `foundation-prioritized-action-plan` (or a user-named chain) and runs an ordered sequence of pm-skills with per-step go/no-go checkpoints (CHECKPOINTED default, GUARDED-AUTO opt-in). The engine delegates through the `Skill` tool (not `Agent`), so it adds zero chain depth and needs no `_chain-permitted` entry. `foundation-prioritized-action-plan` grows to v1.1.0 with an optional `--run` handoff that offers to execute its own runnable prompts through the orchestrator. Catalog 64 to 65 (utility 10 to 11); additive minor, no existing skill changed in behavior. Ships EXPERIMENTAL on all non-Claude clients; the native sub-agent to Skill-tool delegation is build-time smoke-verified before the tag. Previous: **v2.23.0 SHIPPED 2026-05-31** (tag `v2.23.0` at `b54cef0`; GitHub Release Latest; `agent-plugins` registry re-pinned `5f5a3c0`, registry metadata 1.0.2; all post-tag CI green on Linux + Windows) - New foundation skill `foundation-prioritized-action-plan` (evidence-grounded prioritized action plans via Theory of Constraints + Cynefin); catalog 63 to 64 (foundation 8 to 9); additive minor, no existing skill changed. Shipped the full skill package (SKILL.md + references/TEMPLATE + references/EXAMPLE + 2 Complex examples + frameworks/recommendable-tiers/generated skill-catalog + Cynefin eval fixtures) and `scripts/build-skill-catalog.py`. No command wrapper (post-v2.22.0 convention) and no HISTORY.md at launch (created on first iteration). Release hygiene done: counts re-derived 63 to 64 (foundation 8 to 9) across README/docs/AGENTS/manifests; version 2.23.0 across plugin.json + marketplace.json + .codex-plugin + README badge + Current-version row; CHANGELOG [2.23.0] + docs/changelog mirror + Release_v2.23.0.md + releases index + README What's New. Also added the `check-skill-sample-coverage` enforcing CI gate (W2: phase/foundation/tool skills must ship 3 thread samples). Codex adversarial review applied pre-merge; CI caught one stale count in the validator's own doc (fixed via count-exempt). Squash-merged PR #150 to main, tagged, released, registry re-pinned. Previous: **v2.22.0 SHIPPED 2026-05-30** (tag `v2.22.0`) - Wrapper deletion: deleted the 63 command wrappers (the 10 workflow-* commands remain), kept all 63 skill names, retired master-plan D6 (deleted agents/_pairing.yaml), added the Codex .codex-plugin/plugin.json manifest. Previous: **v2.21.0 SHIPPED 2026-05-26** - Marketplace Launch (additive): pm-skills is published through the new `product-on-purpose` marketplace (hosted at the now-public `product-on-purpose/agent-plugins`, a thin registry; recommended home for multiple Product on Purpose plugins) while the self-hosted `pm-skills-marketplace` path is kept alive and working (dual-home). Distribution change only: no skill/command/behavior changes (catalog 63; 73 commands; 4 sub-agents; 12 workflows). MINOR (backward-compatible; nothing removed); the old-path retirement is the breaking step, reserved for a future major. Delivered: tag `v2.21.0` -> commit `1065c3e`, GitHub Release Latest with the rich `Release_v2.21.0.md` body; agent-plugins registry public + branch-protected (requires `validate`, admin bypass), `validate-registry` CI + weekly drift cron, pinned to v2.21.0 via an **https `url` source** (metadata 1.0.0); install instructions repointed to the new marketplace (M1, `ba172a5`). **Two install-path bugs caught + fixed by smoke:** the registry `github` shorthand source cloned the plugin over SSH (broke HTTPS-only users) -> switched to https `url`; the self-hosted marketplace's `source:"./"` was rejected as invalid -> switched to https `url` on main. Smoke S1-S8 cleared on real machines (S1-S5 verified; S6/S7 accepted on reasoned confidence; S8 public-anonymous install PASS). Codex-reviewed clean through 6 passes pre-execution. Plan + companions: `docs/internal/release-plans/v2.21.0/`. Optional remaining: post the staged announcement Discussion (`launch-announcement-draft.md`). Next planned: **v2.22.0** (command/skill naming standardization, additive minor). Previous: **v2.20.0 SHIPPED 2026-05-25** - Sprint Workflow Commands + Validation/Doc Hardening: 3 new sprint `/workflow-` commands (foundation-sprint, design-sprint, foundation-to-design; slash commands 70 -> 73, no new skills); `check-agents-md-command-sync` now requires a command file per `/workflow-` row; `check-count-consistency` gained number-after + singular-noun coverage (caught + fixed stale counts across ecosystem/runtime-components/quickstart docs); vestigial `check-stale-bundle-refs` removed. Shipped via the 6-gate runbook: annotated tag `v2.20.0` -> commit `e1db5ec`, GitHub Release Latest with the rich `Release_v2.20.0.md` body, tag CI green (Validation + Validate Plugin Packaging + Release); Codex G1 review HELD on a P0 (stale ecosystem.md counts), cleared after the fix. Previous: **v2.19.0 SHIPPED 2026-05-23** - pre-promotion hardening: 14 work items (FU-1..FU-9 validator/CI/script hardening + PR-1..PR-5 promotion-readiness) plus a release-prep parity fold-in (wired `validate-version-consistency` into the local pre-tag bundle; the bundle now runs 18 enforcing validators with bash + pwsh parity). Shipped via `/pm-release v2.19.0` 6-gate conductor: annotated tag `v2.19.0` -> commit `a18e4d5` (tag object `fe36694`), GitHub Release published (Latest) with the rich `Release_v2.19.0.md` body, tag CI green (Validation + Validate Plugin Packaging + Release). No new skills (catalog stays 63). Previous: **v2.18.0 SHIPPED 2026-05-22** - four highest-consensus content skills (`discover-market-sizing`, `define-prioritization-framework`, `discover-journey-map`, `measure-survey-analysis`) shipped: annotated tag `v2.18.0` -> `daf720e`, GitHub Release published (Latest), tag CI green (Release + Validate Plugin Packaging). Catalog 59 to 63 (phase 26 to 30); slash commands 66 to 70; 12 new library samples. Aggregates refreshed (AGENTS.md, generated skill pages, README, plugin manifests, docs site); count-consistency + validate-agents-md PASS. Release-prep done (version bump to 2.18.0, CHANGELOG [2.18.0], Release_v2.18.0.md, currency markers, README What's New row, and a correction to a false v2.10.0 What's New entry that mis-attributed these four skills). Validated by the full pre-tag bundle (15 enforcing) PASS, Astro build (363 pages, 0 errors), G1 pm-critic, four Codex adversarial passes, and deep internal verification (~26 findings resolved across commits c763ba3..daf720e). Previous: **v2.17.0 SHIPPED 2026-05-20** via `/pm-release v2.17.0` (6-gate conductor run). Annotated tag `v2.17.0` at commit `b6ff373`; GitHub Release published with the rich Release_v2.17.0.md body; tag CI green (Validate Plugin Packaging + Release workflow). Delivered: native Claude Code sub-agent registration LIVE (`agents/` auto-discovery; `AGENTS/` to `_agent-context/` rename), frontmatter metadata-nested migration across 59 skills, bash-3.2 validator portability, check-version-references advisory wiring. All four work items (W1-W4) + F-P2-01 shipped. Post-tag follow-ups: merge pm-skills-mcp embed-skills.js branch; v2.18.0 (4 content skills) is the next cycle. Previous tagged release: **v2.16.2** (2026-05-19).
...
**Last Updated:** 2026-06-24 (v2.29.1 SHIPPED; G4 post-tag refresh: both currency markers flipped to SHIPPED; the hollow-page fix - gen-site renders every SKILL.md section + an enforcing check-skill-page-sections completeness gate + flat skill cross-reference link resolution). Previously updated 2026-06-23 (v2.29.0 SHIPPED; G4 post-tag refresh: Status + both currency markers flipped to SHIPPED; new foundation-build-risk-review skill + pm-skill-router sub-agent + D8 sample-count gate + M-33 rubric). Previously updated 2026-06-15 (v2.27.0 SHIPPED: the eval program - trigger-accuracy (M-31) + derived surfaces (M-32) + output-quality eval gates (M-33) + creator eval-contract integration + enforcing reciprocity gate; annotated tag at `ee7ff9d5`, GitHub Release Latest; a G1 codex review caught + we fixed 2 fail-closed gaps in the eval gates before the tag). Previously updated 2026-06-10 (v2.26.0 release prep complete: build PRs #187-#197 merged, smoke gate recorded PASS, version surfaces at 2.26.0; tag pending). Previously updated 2026-05-25 (v2.20.0 SHIPPED; G4 post-tag refresh: Status + both currency markers flipped to SHIPPED; 3 sprint workflow commands + count-validator hardening). Previously updated 2026-05-23 (v2.19.0 SHIPPED; G4 post-tag refresh: Status flipped to SHIPPED, validator count to 18, currency markers bumped). Previously updated 2026-05-19 (v2.16.2 SHIPPED; G4 post-tag refresh closing F-P1-02 from v2.16.1 G4 P0 audit. Status block now reflects v2.16.0 to v2.16.1 to v2.16.2 chain). Previously updated 2026-05-18 (v2.16.0 SHIPPED; G4 post-tag CONTEXT.md refresh per master plan + Codex R13 sequencing rule). Previously updated 2026-05-17 late evening (v2.16.0 ready-to-push state on spike branch). Previously updated 2026-05-10 (V12-V15 + A+B+C post-tag work; release docs hygiene sweep at end of v2.14.x cycle).

**Release (v2.14.x archive block):** v2.14.0 SHIPPED 2026-05-10 (Doc Stack Migration). GitHub Release: https://github.com/product-on-purpose/pm-skills/releases/tag/v2.14.0. Previous: v2.13.1 (2026-05-06 plugin install path correction); v2.13.0 (2026-05-05 Foundation Hardening + Doc Stack Decision); v2.12.0 (2026-05-03 OKR Skills Launch); v2.11.1 (2026-04-22); v2.11.0 (2026-04-18 Meeting Skills Family + lean-canvas).

**Skill count (v2.14.x archive block):** 40 (26 phase + 8 foundation + 6 utility). Unchanged from v2.12.0 through v2.14.0; v2.14.0 was a doc-stack migration release with zero catalog changes.
