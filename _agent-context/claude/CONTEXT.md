...
**Status:** **v2.18.0 RELEASE-PREP (2026-05-21)** - four highest-consensus content skills (`discover-market-sizing`, `define-prioritization-framework`, `discover-journey-map`, `measure-survey-analysis`) authored and validated. Catalog 59 to 63 (phase 26 to 30); slash commands 66 to 70; 12 new library samples. Aggregates refreshed (AGENTS.md, generated skill pages, README, plugin manifests, docs site); count-consistency + validate-agents-md PASS. Release-prep done (version bump to 2.18.0, CHANGELOG [2.18.0], Release_v2.18.0.md, currency markers, README What's New row, and a correction to a false v2.10.0 What's New entry that mis-attributed these four skills). PENDING before tag: full pre-tag bundle + G1 pm-critic review, then commit/push/tag await maintainer ship-it. Previous: **v2.17.0 SHIPPED 2026-05-20** via `/pm-release v2.17.0` (6-gate conductor run). Annotated tag `v2.17.0` at commit `b6ff373`; GitHub Release published with the rich Release_v2.17.0.md body; tag CI green (Validate Plugin Packaging + Release workflow). Delivered: native Claude Code sub-agent registration LIVE (`agents/` auto-discovery; `AGENTS/` to `_agent-context/` rename), frontmatter metadata-nested migration across 59 skills, bash-3.2 validator portability, check-version-references advisory wiring. All four work items (W1-W4) + F-P2-01 shipped. Post-tag follow-ups: merge pm-skills-mcp embed-skills.js branch; v2.18.0 (4 content skills) is the next cycle. Previous tagged release: **v2.16.2** (2026-05-19).
...
**Validators:** the `scripts/pre-tag-validate.sh` bundle runs 15 enforcing validators (11 required + 4 preventive) plus 1 non-blocking advisory (`check-version-references`, wired in v2.17.0 F-P2-01). v2.17.0 W3 made 8 of the bundle validators bash-3.2 compatible (macOS default bash) and added a non-git-work-tree fallback to check-count-consistency. Full inventory + workflow mapping in `docs/contributing/ci-overview.md`. (Note: the broader tracked validator-script count, including .ps1 mirrors and CI-only validators outside the bundle, is larger; the bundle is the canonical enforcing surface.)
