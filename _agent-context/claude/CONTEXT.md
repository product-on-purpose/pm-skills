...
**Status:** Last tagged release **v2.16.2** (2026-05-19, SHIPPED). **v2.17.0 is staged on `main`, untagged** - all four work items complete; the release cut (version bump + CHANGELOG + tag) is pending via the `/pm-release v2.17.0` 6-gate runbook.

**v2.17.0 (in progress, untagged on main):** W1 frontmatter metadata migration across 59 skills (`be1352b`, CI-green). W2 `AGENTS/` to `_agent-context/` + `subagents/` to `agents/` rename (`3691b77`); native Claude Code sub-agent registration is LIVE - all 4 sub-agents auto-discover via `@`-mention (attested 2026-05-20 after `/plugin update` + `/reload-plugins`); phantom `agents/README.md` removed (`e162ae0`). W3 bash-3.2 validator portability across 8 validators + check-count-consistency non-git fallback (`6b097b9`). F-P2-01 wired `check-version-references` into the pre-tag bundle as a non-blocking advisory step (`6b097b9`). W4 = this CONTEXT refresh. Full pre-tag bundle 15/15 enforcing PASS; Astro build 345 pages, 0 errors. Remaining for the tag: version bump (plugin.json/marketplace.json/README badge to 2.17.0), CHANGELOG v2.17.0 entry, Release_v2.17.0.md, then `/pm-release v2.17.0`. NOTE: bump this CONTEXT currency marker to v2.17.0 at the version-bump step so check-context-currency stays consistent with CHANGELOG.
