...
**Do not add a new `scripts/<name>.sh` + `scripts/<name>.ps1` validator pair.** Write new validators as a single cross-platform Node script (`scripts/<name>.mjs`) with a matching `scripts/<name>.test.mjs`, wired directly into `.github/workflows/validation.yml` so it runs once on both OS legs. This is how every recent gate ships (`check-frontmatter-yaml`, `check-rendered-links`, `check-route-parity`, `check-count-phrases`, `check-heading-canon`, the trigger/collision evals, and the ported `check-count-consistency` are all single-source Node).
...
Porting has begun (v2.31.0, WS-Z4; audit D-1 ruled B-complete): `check-count-consistency` is now single-source `scripts/check-count-consistency.mjs`, proven byte-identical to both retired shells against the committed fixture tree in `scripts/fixtures/shell-parity/repo` before deletion (that fixture is now the port's integration corpus, and the interim `shell-parity-smoke.mjs` guard retired with the pair it watched). The remaining dual-shell pairs are ported 1-2 per release until none remain. If you must touch a still-dual-shell validator before it is ported, edit BOTH scripts and keep the awk hazard comments intact.
